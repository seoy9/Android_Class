...
//    나라 이름으로 DB 검색
    public ArrayList<Food> getFoodsByNation(String nation) {
        SQLiteDatabase db = foodDBHelper.getReadableDatabase();
        String[] colums = {null};
        String selection = "nation=?";
        String[] selectArgs = new String[] {nation};

        Cursor cursor = db.query(FoodDBHelper.TABLE_NAME, colums, selection, selectArgs, null, null, null, null);
        String result = "";

        while(cursor.moveToNext()) {
            String food = cursor.getString(cursor.getColumnIndex("title"));
            String foodNation = cursor.getString(cursor.getColumnIndex("nation"));
            result += food + " - " + foodNation + "\n";
        }
        cursor.close();
    }
